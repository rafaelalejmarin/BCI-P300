...
with open(name,'w+') as f:
    ...
    for fileIndex in range(len(testfiles)):
        ...
        if nums:
            f.write(str(nums) + '\n')
        f.write('precision, usefulness, sensitivity, specificity, truePositive, falsePositive, trueNegative, falseNegative')
        ...
        for thrs in th:
            ...
            try:
                result = acc(test_result.numpy(), test_new[10,:], 5)
                exp = '\n\n' + orders[order] + 'order, threshold = ' + str(thrs) + ', net number = '+str(netnum) + ' 3x3  conv, 10 iterations, batchsize = ' + str(batch_size) + '\n\n'
                f.write(exp)
                f.write(str(result))
