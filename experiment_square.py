    ...
    for thrs in th:
        for num_iter in range(3):
            ...
            try:
                ...
                exp = '\n\n standard order, threshold = ' + str(thrs) + ', net number = '+str(netnum) + ' 3x3  conv, 10 iterations, batchsize = ' + str(batch_size) + '\n\n'
                f.write(exp)
                f.write(str(result))
            except (ZeroDivisionError):
                print('zero predicted positive indices')
                f.write('\n\nzero predicted positive indices for num_layers = ' + str(netnum)+'\n')
                pass
            end = time.time()
            print(end-start)
            f.write('\n')           
